...
{
    private static final String DOCUMENTS_PATH = "inforetr/movies/movies/";
    ...
    private void initModelsStemmerLemmatizer()
    {
        try
        {
            _sentenceModel = new SentenceModel(Paths.get(System.getProperty("user.dir")).resolve(OpenNLP.SENTENCE_MODEL).toFile());
            _tokenizerModel = new TokenizerModel(Paths.get(System.getProperty("user.dir")).resolve(OpenNLP.TOKENIZER_MODEL).toFile());
            _lemmatizer = new DictionaryLemmatizer(Paths.get(System.getProperty("user.dir")).resolve(OpenNLP.LEMMATIZER_DICT).toFile());
            _stemmer = new PorterStemmer();
            _posModel = new POSModel(Paths.get(System.getProperty("user.dir")).resolve(OpenNLP.POS_MODEL).toFile());
            _peopleModel = new TokenNameFinderModel(Paths.get(System.getProperty("user.dir")).resolve(OpenNLP.NAME_MODEL).toFile());
            _placesModel = new TokenNameFinderModel(Paths.get(System.getProperty("user.dir")).resolve(OpenNLP.NAME_MODEL).toFile());
            _organizationsModel = new TokenNameFinderModel(Paths.get(System.getProperty("user.dir")).resolve(OpenNLP.NAME_MODEL).toFile());
    ...
    private void processFile(String text)
    {
        ...
        NameFinderME orgsFinder = new NameFinderME(_organizationsModel);
        ...
        int noSentences = sentences.length;//_sentenceModel.getEosCharacters().length;
        //    - number of tokens
...
//        _totalTokensCount+=noTokens;
        ...
        String[] lemmatize = _lemmatizer.lemmatize(text.split(" "), tokens);

        //    -  people
        Span people[] = peopleFinder.find(tokens); //new Span[] { };
        //    - locations
        Span locations[] = placesFinder.find(tokens);
        //    - organisations
        Span organisations[] = orgsFinder.find(tokens);
...
//                DONE
        ...
        // TODO derive tokens and POS tags from text // DONE tokens & posTags
        //tokens var
        ...
        // TODO update overall statistics - use tags and check first letters // DONE
        ...
        for(String s : sentences) {
            String[] tempTokens=tokenizerME.tokenize(s);
            String[] posTags = posTaggerME.tag(tempTokens);
            for (String posTag : posTags) {
                _totalTokensCount++;
                if (posTag.startsWith("V")|| posTag.startsWith("M"))
                    _verbCount++;
